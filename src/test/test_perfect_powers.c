...
static bool check_power(uint64_t b, uint64_t x){
	...
	if(base_a != b){
		...
		return false;
	}else if(is_perfect_power(b, 31, &base_b, &exp_b)){
...
int main(){
	...
	for(uint64_t x = 3, prev_r = 0; x <= 64; ++x){
		uint64_t r = u64_nth_root(UINT64_MAX, x);
