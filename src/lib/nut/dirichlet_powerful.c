...
static bool PowerfulStack_init(PowerfulStack *self, uint64_t max){
	...
	// TODO: clang does not fully support C23 typeof yet
	self->entries[0] = (__typeof__(*self->entries)){.n=1, .hn=1, .i=0};
	self->len = 1;
	return true;
}
...
static bool PowerfulStack_push(PowerfulStack *self, __typeof__(self->entries) ent){
...
static bool PowerfulStack_pop(PowerfulStack *self, __typeof__(self->entries) out){
	if(!self->len){
		return false;
	}
...
bool nut_Diri_sum_adjusted_he(int64_t *restrict out, int64_t m, const nut_Diri *restrict g_tbl, const int64_t *restrict h_vals){
	...
	int64_t *g_dense = malloc((g_tbl->y + 1)*sizeof(int64_t));
	uint64_t xr = nut_u64_nth_root(g_tbl->x, 2);
	uint64_t num_primes;
	uint64_t *primes = nut_sieve_primes(xr, &num_primes);
	PowerfulStack gens = {};
	PowerfulStack_init(&gens, g_tbl->x);
	if(!g_dense || !primes || !gens.entries){
		free(g_dense);
		free(primes);
		PowerfulStack_destroy(&gens);
		return false;
	}
	g_dense[0] = 0;
